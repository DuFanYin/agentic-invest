...
_RATE_CHANGE_THRESHOLD = 0.01  # minimum move (percentage points) to signal direction
_PRICE_CHANGE_THRESHOLD_PCT = 0.5  # minimum % move to signal bullish/bearish
...
def _fetch_fred_series(series_id: str, label: str, api_key: str) -> dict[str, Any]:
    ...
    try:
        ...
        if prev:
            ...
            if value > prev_val + _RATE_CHANGE_THRESHOLD:
                direction = "rising"
            elif value < prev_val - _RATE_CHANGE_THRESHOLD:
                direction = "falling"
        return {
            "series_id": series_id,
            "label": label,
...
def _fetch_yf_macro(ticker: str, label: str) -> dict[str, Any]:
    try:
        ...
        tk = yf.Ticker(ticker)
        hist = retry_sync(
            lambda: tk.history(period="5d", interval="1d"), retry_on=(Exception,), op_name=f"yfinance.macro.{ticker}"
        )
        if hist.empty:
        ...
        if change_pct > _PRICE_CHANGE_THRESHOLD_PCT:
            direction = "rising"
        elif change_pct < -_PRICE_CHANGE_THRESHOLD_PCT:
            direction = "falling"
        return {
            "ticker": ticker,
            "label": label,
...
class MacroDataClient:
    """FRED + yfinance macro signals. Pass a shared ``Cache`` (e.g. from ``build_graph``) for one pool per request."""

    def __init__(self, *, cache: Cache | None = None) -> None:
        self._cache = cache or Cache(db_path=CACHE_DB_PATH)
    ...
    async def get_fred_indicators(self) -> dict[str, Any]:
        ...
        if not FRED_API_KEY:
            ...
            return {}
        cached = await asyncio.to_thread(self._cache.get, "fred:indicators")
        if cached:
            return cached
        tasks = {
        ...
        await asyncio.to_thread(self._cache.set, "fred:indicators", data, ttl_seconds=_FRED_TTL)
        return data

    async def get_market_signals(self) -> dict[str, Any]:
        """Return recent price stats for macro yfinance tickers. Cached 15min."""
        cached = await asyncio.to_thread(self._cache.get, "macro:market_signals")
        if cached:
            return cached
        tasks = {ticker: asyncio.to_thread(_fetch_yf_macro, ticker, label) for ticker, label in MACRO_TICKERS.items()}
        results = await asyncio.gather(*tasks.values(), return_exceptions=True)
        data: dict[str, Any] = {}
        for ticker, result in zip(tasks.keys(), results):
            if isinstance(result, Exception):
                ...
                data[ticker] = result
        await asyncio.to_thread(self._cache.set, "macro:market_signals", data, ttl_seconds=_MARKET_TTL)
        return data

    async def get_all(self) -> dict[str, Any]:
        """Fetch FRED indicators and market signals concurrently."""
        fred, signals = await asyncio.gather(self.get_fred_indicators(), self.get_market_signals())
