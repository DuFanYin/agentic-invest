...
def test_iteration_limit_fires_at_max():
    dec = evaluate_policy(_inp(research_iteration=2, max_iterations=2, judge_reason="structural", judge_retry_question="fix ticker"))
    assert dec.action == "continue"
    assert dec.reason_code == "iteration_limit"


def test_iteration_limit_does_not_fire_below_max():
    dec = evaluate_policy(_inp(research_iteration=1, max_iterations=2, judge_reason="structural", judge_retry_question="fix ticker"))
...
def test_partial_degraded_does_not_halt():
    dec = evaluate_policy(_inp(fa_degraded=True, macro_degraded=False, ms_degraded=False))
...
def test_all_degraded_beats_evidence_conflict():
    ...
    dec = evaluate_policy(_inp(
        fa_degraded=True, macro_degraded=True, ms_degraded=True,
