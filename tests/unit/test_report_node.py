    ...
    return FundamentalAnalysis.model_validate(
        {
            ...
            "fundamental_risks": [
                {"name": "Margin risk", "impact": "medium", "signal": "GM declining", "evidence_ids": ids}
            ],
            "missing_fields": [],
            "metrics": {},
        }
    ...
    return MarketSentiment.model_validate(
        {
            ...
            "sentiment_risks": [
                {"name": "Reversal risk", "impact": "low", "signal": "weak guidance", "evidence_ids": ids}
            ],
            "missing_fields": [],
        }
    )
...
def _mock_llm(raises: Exception | None = None):
    llm = MagicMock()
    if raises:
        llm.complete_text = AsyncMock(side_effect=raises)
    else:
        llm.complete_text = AsyncMock(return_value="## Section\n\nContent here.")
    return llm
...
def test_debate_degraded_produces_warning():
    ...
    state["scenario_debate"] = ScenarioDebate(
        ...
        calibrated_scenarios=[
            CalibratedScenario(name=s.name, probability=s.probability, tags=list(s.tags)) for s in _scenarios()
        ],
        confidence="low",
